...
final class Version20250612133230 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        <p>Je suis donc passé sur un VPS Debian chez Infomaniak que j\'ai totalement configuré from-scratch. Cette migration représentait un défi technique intéressant pour approfondir mes connaissances en administration système.</p>
        <h2>Sécurisation du serveur</h2>
        <p>Les premières étapes de mise en place concernaient l\'installation et la sécurisation du VPS : update et upgrade du système, mise en place du pare-feu ufw couplé au pare-feu Infomaniak, installation de fail2ban pour la protection contre les attaques par force brute, et configuration des règles d\'authentification SSH sécurisées.</p>
        <h2>Architecture d\'hébergement</h2>
        <p>J\'ai ensuite mis en place une architecture robuste pour l\'hébergement des applications : nginx global pour gérer les certificats SSL, les headers de sécurité et les redirections vers les domaines et sous-domaines. Chaque application dispose de son propre conteneur Docker avec un nginx spécifique qui redirige via FastCGI vers l\'application PHP-FPM. Un conteneur Docker MySQL avec un réseau partagé permet à tous les conteneurs d\'applications de communiquer entre eux.</p>
        <h2>Monitoring avancé</h2>
        <p>J\'ai développé un script de monitoring personnalisé pour obtenir des statistiques en temps réel : consommation et performances du serveur, statut des domaines et sous-domaines, validité des certificats SSL, rapports de sécurité (fail2ban, connexions récentes), et de nombreuses autres métriques essentielles.</p>
        <h2>Conclusion</h2>
        ...
        <p>So I switched to a Debian VPS at Infomaniak that I completely configured from scratch. This migration represented an interesting technical challenge to deepen my system administration knowledge.</p>
        <h2>Server Security</h2>
        <p>The first setup steps concerned VPS installation and security: system update and upgrade, ufw firewall setup coupled with Infomaniak\'s firewall, fail2ban installation for brute force attack protection, and secure SSH authentication rules configuration.</p>
        <h2>Hosting Architecture</h2>
        <p>I then set up a robust architecture for application hosting: global nginx to manage SSL certificates, security headers and redirections to domains and subdomains. Each application has its own Docker container with specific nginx that redirects via FastCGI to the PHP-FPM application. A Docker MySQL container with shared network allows all application containers to communicate with each other.</p>
        <h2>Advanced Monitoring</h2>
        <p>I developed a custom monitoring script to get real-time statistics: server consumption and performance, domain and subdomain status, SSL certificate validity, security reports (fail2ban, recent connections), and many other essential metrics.</p>
        <h2>Conclusion</h2>
        ...
        $this->addSql("INSERT INTO articles (
            ...
            'hosting-migration-from-o2switch-to-a-self-managed-debian-vps',
            'Migration d\'hébergement web : passage d\'o2switch vers un VPS Debian auto-configuré avec Docker, nginx, monitoring personnalisé et architecture sécurisée.',
            'Web hosting migration: transition from o2switch to a self-configured Debian VPS with Docker, nginx, custom monitoring and secure architecture.',
